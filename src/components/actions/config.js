...
export function AC_ADD_CONFIGURATION(userData) {
    ...
    return function (dispatch) {
        return axios.post("http://localhost:8000/api/v1/config/addUpdateConfig", userData,config)
            .then(({ data }) => {
...
export function AC_LIST_CONFIGURATION() {
    return function (dispatch) {
        return axios.get("http://localhost:8000/api/v1/config/listConfig",config)
            .then(({ data }) => {
...
export function AC_DELETE_CONFIGURATION(formdata){
    ...
    return function(dispatch){
        return axios.post("http://localhost:8000/api/v1/config/deleteConfig",formdata,config)
        .then(({data}) => {
...
export function AC_VIEW_CONFIGURATION(formdata){
    console.log('===-=-=action -=-=-',formdata)
    return function(dispatch){
        return axios.post("http://localhost:8000/api/v1/config/viewConfig",formdata,config)
        .then(({data}) => {
